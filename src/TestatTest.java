...
public class TestatTest {
	...
	public void testPerformance() {
		int NUM_UNIQUE_ELEMENTS_IN_CATALOGUE = 15196;
		...
		assertEquals(NUM_UNIQUE_ELEMENTS_IN_CATALOGUE - firstTerm.size(),
				mapper.size());
		...
		secondTerm.stream().forEach(p -> termTwoModuleNames.add(p.name));
		...
		lastTerm.stream().forEach(p -> termLastModuleNames.add(p.name));
